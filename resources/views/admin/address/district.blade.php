...
@section('content')
  ...
  <div class="sl-pagebody">
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <form class="form-horizontal" id="registration" method="post" action="{{ (@$data)?route('district.update') : route('district.store') }}" enctype="multipart/form-data">
              @csrf
              <div class="card">
                  ...
                  <div class="card-body card_form">
                    ...
                    <div class="form-group row custom_form_group{{ $errors->has('DistName') ? ' has-error' : '' }}">
                        <label class="col-sm-3 control-label">District Name:<span class="req_star">*</span></label>
                        <div class="col-sm-7">
                          <input type="text" placeholder="District Name" class="form-control" id="DistName" name="DistName" value="{{(@$data)?@$data->DistName:old('DistName')}}">
                          <input type="hidden" name="DistId" value="{{@$data->DistId ?? ''}}">
                          @if ($errors->has('DistName'))
                              <span class="invalid-feedback" role="alert">
                                  <strong>{{ $errors->first('DistName') }}</strong>
                              </span>
                        ...
                        <div class="col-12">
                            <div class="table-responsive">
                                ...
                                <table id="datatable1" class="table responsive mb-0">
                                    ...
                                    <tbody>
                                      @foreach ($allDist as $key=>$dist)
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ $dist->Division->DiviName ??'' }}</td>
                                            <td>{{ $dist->DistName ??'' }}</td>
                                            <td>
                                                <a href="#" title="view"><i class="fa fa-plus-square fa-lg view_icon"></i></a>
                                                <a href="{{ route('district.edit',$dist->DistId) }}" title="edit"><i class="fa fa-pencil-square fa-lg edit_icon">Edit</i></a>
